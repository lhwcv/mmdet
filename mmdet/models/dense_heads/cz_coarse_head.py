...
class CZ_CoarseHead(BaseDenseHead, BBoxTestMixin):
    """
    ...
    def __init__(self,
                 ...
                 anchor_generator=dict(
                     ...
                     base_sizes=[[ (32,32), (48, 48),(24,32), (32, 48) ],
                           [ (64, 64),(72, 72),(72,96),(96,96) ],
                           [ (72,96),(96,96),(128,128),(96,128) ]],
                     strides=[16, 32, 64]),
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            conv_pred = nn.Conv2d(self.in_channels[i],
                                  self.num_anchors * self.num_attrib,
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        pred_x1y1 = pred_map[..., :2] - pred_map[..., 2:4] / 2
        pred_x2y2 = pred_map[..., :2] + pred_map[..., 2:4] / 2
        pred_xyxy = torch.cat((pred_x1y1,pred_x2y2),dim=-1)
        target_x1y1 = target_map[..., :2] - target_map[..., 2:4] / 2
        target_x2y2 = target_map[..., :2] + target_map[..., 2:4] / 2
        target_xyxy = torch.cat((target_x1y1, target_x2y2), dim=-1)
        ...
        #print('pred_xyxy shape: ', pred_xyxy.shape)
        #print('target_xyxy shape: ', target_xyxy.shape)
        #print('weight shape: ', pos_mask.shape)
#         pos_mask= pos_mask.repeat((1,1,4))
#         pred_xyxy  = pred_xyxy.reshape((-1,4))
#         target_xyxy = target_xyxy.reshape((-1, 4))
#         pos_mask = pos_mask.reshape((-1, 4))
#         loss_iou = self.loss_iou(pred_xyxy, target_xyxy,weight=pos_mask )
#         return loss_cls, loss_conf, loss_xy, loss_wh,loss_iou
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes,
        ...
        #print('concat_anchors shape: ',concat_anchors.size())
        #print('gt_bboxes shape: ', gt_bboxes.size())
        # assign_result = self.assigner.assign(concat_anchors,
        #                                      concat_responsible_flags,
        #                                      gt_bboxes)

        assign_result = self.assigner.assign(
            concat_anchors, gt_bboxes, concat_responsible_flags,
